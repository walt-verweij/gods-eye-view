...
`POST /api/realtime/debug-log` (`server/proxies/openaiRealtime.mjs`) appends any JSON body,
...
- `server/proxies/gbfs.mjs` logs `error.message`; URL validation still admits userinfo on an
  allowed host, and a rejected URL is echoed whole. Done: reject userinfo, log a fixed code.
- `server/proxies/opensky.mjs` logs the raw OAuth `error_description`. Done: log status plus an
  allowlisted error code.
- `server/proxies/cctv.mjs` (three sites) and `server/proxies/firms.mjs` log raw parse or
  transport messages that can carry source snippets or echoed keys. Done: fixed codes only.
- `server/proxies/opensky.mjs`, `server/proxies/aisLive.mjs` (via `src/data/aisWatchdog.js`),
...
Milestone 3 built `server/cctvTransport.mjs` and milestone 7 folded the address classifier into
`server/shared.mjs`, but only CCTV frame and media fetches and the radio proxy use it. Every
