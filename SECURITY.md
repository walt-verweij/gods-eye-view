...
The data proxies under `server/providers/` (composed by `server/providers/local.js` and registered by `server/standalone/vite.config.js`) are written so the browser cannot turn the server into an open relay:
