/** Dev-only in-app provider-key setup endpoint. */
...
import { admitKeySetupRequest, isKeySetupExternallyManaged, keySetupStatus, upsertDotenvValues, validateKeySetupUpdates } from '../../src/keySetupCore.mjs';
 ...
 */
export function keySetupEndpoint({ rootDir, launcherAtBoot, providerEnvAtBoot, devFreshExternalKeysAtBoot }) {
  ...
  const pinokioManaged = () => launcherAtBoot === 'pinokio';
  const storeName = () => (pinokioManaged() ? 'pinokio-environment' : 'env-file');
  const storePath = () => path.join(rootDir, ...(pinokioManaged() ? ['pinokio', 'ENVIRONMENT'] : ['.env']));
  ...
  const isExternallyManaged = (name, inStore) => {
    const wasExternalAtBoot = pinokioManaged()
      ? false
      : launcherAtBoot === 'dev-fresh'
        ? devFreshExternalKeysAtBoot.has(name)
        : providerEnvAtBoot[name] !== '';
