/** OpenAI Realtime token and HUD-summary proxy.
 * Outbound guard gap: OpenAI API fetches are not yet routed through it.
 */
...
import { clientKey, enforceOptInRateLimit, makeOptInRateLimiter, registerProxy } from '../shared.mjs';
...
function openAiRateLimiter() {
  if (_openAiRateLimiter === undefined) {
    _openAiRateLimiter = makeOptInRateLimiter(process.env.GEV_RATELIMIT_OPENAI_PER_MIN);
  }
